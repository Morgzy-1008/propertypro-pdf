...
async function runTest() {
  ...
  console.log('Selecting Avalon Facade...');
  ...
  await page.screenshot({ path: path.join(__dirname, '../media_verify_amber21_avalon_fixed.png'), fullPage: true });

  // 2. Double Storey: Burgundy 27 + Allure Facade
  ...
  console.log('Selecting Allure Facade...');
  await page.locator('button:has-text("Browse facade library")').click();
  await page.waitForTimeout(500);
  await page.locator('[role="dialog"] button:has-text("Allure")').first().click();
  await page.waitForTimeout(2000);

  await page.screenshot({ path: path.join(__dirname, '../media_verify_burgundy27_allure_fixed.png'), fullPage: true });
